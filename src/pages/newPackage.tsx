...
import { savePackage } from 'services/package'
...
const NewPackage: NextPage = () => {
	...
	const [packageNameCreated, setPackageNameCreated] = useState('')

	const handleClose = () => setOpenModalSuccess(false)

	const validationSchema = yup.object({
		title: yup.string().required('Nome do pacote é obrigatório'),
		participants: yup
			.number()
	...
	const formik = useFormik({
		initialValues: {
			title: `Pacote para ${destinations[1].label}`,
			participants: 9,
			destination: 'gramado',
		...
		onSubmit: async (values) => {
			try {
				...
				setPackageNameCreated(response?.title ?? '')
			...
			>
				<Box sx={style}>
					...
					<Typography id="modal-modal-description" sx={{ mt: 2 }}>
						Novo pacote &quot;{packageNameCreated}&quot; criado com sucesso.
					</Typography>
				</Box>
			</Modal>
		</>
